...
def get_model(model_name = 'efficientnet-b0', lr = 1e-5, wd = 0.01, feature_extracting = False, opt_fn=torch.optim.AdamW, device = None, n_classes = 1):
...
def training_step(xb, yb, model, loss_fn, opt, device, scheduler):
	...
	return loss.item(), nn.Softmax(dim=1)(out)
